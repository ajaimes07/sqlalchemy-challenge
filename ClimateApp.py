...
# g.  Return a JSON list of stations from the dataset.
...
# h. Return the JSON representation of your dictionary.
    return jsonify(stnsNo_total)

@app.route("/api/v1.0/tobs")
def tobs():
    print("Server received request for 'Precipitation' page...")
# i. Query for the dates and temperature observations from a year from the last data point.
    ...
    filter(Measurement.date > qr2).\
    order_by(Measurement.date).all()
# j. Convert the query results to a Dictionary using `date` as the key and `prcp` as the value.
    AT=[]
    for result in tobs:
        tobs_dict = {}
        ...
        AT.append(tobs_dict)
# k. Return a JSON list of Temperature Observations (tobs) for the previous year.
    return jsonify(AT) 
